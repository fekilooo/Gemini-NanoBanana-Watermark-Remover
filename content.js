(() => {
  ...
  // Request Background script to fetch the blob
      ...
      return new Promise((resolve, reject) => {
          chrome.runtime.sendMessage({ action: "fetchBlob", url: url }, (response) => {
              ...
              if (response && response.success) {
                  ...
                  reject(new Error(response.error || 'Unknown error in background fetch'));
              }
          });
      });
  }

  async function copyCleanImageToClipboard(imgElement, btnElement) {
    ...
    try {
      ...
      const cleanUrl = replaceWithNormalSize(originalSrc);

      const blob = await fetchBlobViaBackground(cleanUrl);
      const blobUrl = URL.createObjectURL(blob);
      const rawImg = await loadImage(blobUrl);

      const processedCanvas = await engine.removeWatermarkFromImage(rawImg);
      ...
      setTimeout(() => { btnElement.innerText = originalText; btnElement.style.cursor = "pointer"; }, 2000);

    } catch (error) {
      console.error("Copy failed:", error);
      btnElement.innerText = "❌ 失敗";
      setTimeout(() => { btnElement.innerText = originalText; }, 2000);
    }
  }

  function injectCopyButton(imgElement) {
    if (imgElement.dataset.hasCopyBtn === "true") return;

    let container = imgElement.parentElement;
    if (!container) return;
    
    // Check if inside picture tag
    ...
    const showBtn = () => { btn.style.opacity = '1'; btn.style.pointerEvents = 'auto'; };
    const hideBtn = () => { btn.style.opacity = '0'; btn.style.pointerEvents = 'none'; };

    container.addEventListener('mouseenter', showBtn);
    container.addEventListener('mouseleave', hideBtn);
    container.addEventListener('touchstart', showBtn);

    btn.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        copyCleanImageToClipboard(imgElement, btn);
    };

    container.appendChild(btn);
    imgElement.dataset.hasCopyBtn = "true";
  }

  var isValidGeminiImage = (img) => {
      if (!img.src.includes("googleusercontent.com")) return false;
  ...
  var processAllImages = () => {
    ...
    images.filter(isValidGeminiImage).forEach(injectCopyButton);
  };
  ...
  var setupMutationObserver = () => {
    const observer = new MutationObserver((mutations) => {
        ...
        if (shouldProcess) processAllImages();
    });
    observer.observe(document.body, { childList: true, subtree: true });
  };

  (async function init() {
    console.log("[Gemini Extension] Starting...");
